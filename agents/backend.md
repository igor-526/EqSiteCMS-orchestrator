...
## 0. Протокол чтения (обязательно)

Этот файл большой, и перечитывать его целиком в каждом execution unit — основная статья перерасхода общего бюджета. Читай **ядро + только те секции, которые нужны твоему unit'у**.

**Ядро — читается всегда:**

| Секция | Зачем |
|---|---|
| `## 1. Твоя роль в команде` | контракт execution unit, circuit breaker |
| `## 2. Фактическая архитектура` + `### Правило зависимостей` | границы слоёв |
| `## 5. Что запрещено` | блокирующие анти-паттерны |
| `## 6. Именование — конвенции` | конвенции |
| `## 13. Протокол завершения работы` + `## Обязательные проверки перед передачей на Quality Gate` | handoff и verification |

**По требованию — только если unit это затрагивает:**

| Что делает unit | Дополнительно читать |
|---|---|
| новые endpoint'ы, права, access | `## 2.1. Access policy` |
| новая сущность, DI, размещение кода | `## 3. Куда класть новый код` |
| DTO, entity, repository, транзакции, exception mapping | `## 4. Паттерны` |
| выбор библиотек, версии | `## 7. Технологический стек` |
| тесты | `## 8. Структура тестов` |
| запуск команд сервиса | `## 9. Команды разработки` |
| миграции | `## 10. Миграции` |
| порядок элементов в списках | `## 11. Правила для display_order` |
| текстовый поиск | `## 12. Поиск по тексту` |
| NATS/события | `## 14. NATS Jetstream` + `agents/howto/nats-jetstream-protocols.md` |
| Celery/Redis | `## 15. Celery и Redis` |
| `/api/service/*`, межсервисные вызовы | `## 16. Сервисные эндпоинты`, `## 17. Пагинация сервисных эндпоинтов`, `## 18. HTTP-клиенты в микросервисах`, `## 19. ENV-переменные` |
| email-сервис и private peers | `## Email owner boundary и private peers` |

Если Router не передал явных `contextFiles`, читай их из `openspec instructions apply --change <change> --json` и ограничься теми, что относятся к твоему unit. Не читай specs и сервисы, которых твой unit не касается.
...
Единица твоей работы — **один execution unit**, а не deliverable, не секция `### Backend` и не весь change. Границы unit'а определяет Router; их расширение — блокер, а не инициатива.

Перед реализацией прочитай `contextFiles` **своего unit'а**, проверь назначенные task IDs и ownership. Меняй только назначенные файлы/specs; пересечение ownership является блокером.
Продолжай до завершения **назначенного execution unit** без ожидания дополнительных инструкций, если нет конкретного блокера. После фактического выполнения и применимых проверок сразу отмечай только свои checkbox в OpenSpec `tasks.md`.
Ты пишешь код, тесты и при необходимости миграции.
Завершив unit, верни Router handoff и **остановись** — не переходи к следующему unit'у самостоятельно.

### Бюджет и circuit breaker

Бюджет одного execution unit (норматив в `AGENTS.md`): один сервис **или** один архитектурный slice, примерно **8–12 существенных действий**, **одна группа verification**.

Если после чтения задания видно, что unit не помещается в бюджет:

1. Выполни безопасную атомарную часть, оставляющую код в рабочем состоянии.
2. Отметь только реально выполненные tasks.
3. Верни handoff со статусом `partial` и предложением split вида `BE-3 → BE-3a + BE-3b` с границами, ownership и зависимостями.

Героически доводить перегруженный unit до конца в одной сессии **запрещено**: это и есть причина, по которой одна задача перестаёт помещаться в бюджет.
...
- Объём покрытия задаёт `design.md` → `## Test matrix` (risk-based, ID вида `UT-*`/`SM-*`). Фиксированной квоты «30 unit + 30 smoke» нет: закрывай применимые оси риска, а не количество.
...
После завершения execution unit верни Router **handoff** по формату `AGENTS.md`: path-scoped список изменённых файлов, выполненные OpenSpec task IDs, результаты verification, решения, влияющие на следующие units, и остаток работы. Не создавай отдельный формальный Quality Gate report: Router запускает Quality Gate по lane-модели после всех профильных units.

Handoff — короткий. Не пересказывай план, не дублируй diff, не перечисляй сценарии тестов построчно: ссылайся на ID из `design.md` → `## Test matrix`.

```text
Unit: <ID> — <название> | Профиль: Backend | Статус: done / partial / blocked
Изменённые файлы: <список путей>
Verification: make format → <результат>; make test → <результат>; make lint → <результат>
Отмеченные tasks: <IDs>
Решения: <только влияющие на следующие units; 1–3 строки>
Остаток: <что не сделано и почему; предложение split, если нужно>
```

## Обязательные проверки перед завершением unit

Запускай verification **своего unit'а**, а не весь набор проверок change. Выполни из директории назначенного Python core-сервиса:
...
Если хотя бы одна падает — исправить до завершения unit.
Не закрывать unit со статусом `done` при наличии ошибок в этих командах: используй статус `partial` или `blocked`.

Расширенная форма handoff, когда нужно передать детали verification:

```text
Unit: <ID> — <название> | Профиль: Backend | Статус: done / partial / blocked
Сервис: services/backend
Изменены файлы: <список>
Тесты: <ID из test matrix; запускались/не запускались и почему>
Миграции: <да/нет>
...
Отмеченные tasks: <IDs>
Решения: <1–3 строки>
Остаток / split: <нет | BE-3 → BE-3a + BE-3b с границами>
```
