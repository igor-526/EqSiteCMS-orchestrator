...
### 1.1. Контуры потребителей API (обязательно)

- `services/frontend` — CMS-контур (администрирование, авторизованные сценарии).
- `site-ad` и аналогичные сайты — публичный контур потребления read API (anonymous `GET`).

Frontend-агент обязан учитывать, к какому контуру относится экран/фича, и не смешивать эти контуры.
...
- Публичные страницы и виджеты (сайтовый контур) используют только public read endpoint'ы.
- CMS-сценарии изменения данных используют только protected write endpoint'ы с авторизованным контекстом.
- Нельзя привязывать публичный UI к CMS-only endpoint'ам.
- Для каждого нового пользовательского сценария проверь соответствие `UI flow -> endpoint access class` (public/protected) и зафиксируй это в плане/отчете.
