...
| Секция | Зачем |
|---|---|
| `## 1. Роль и контуры` | контракт execution unit, circuit breaker |
| `## 2. Фактическая архитектура` + `### 2.1. Куда класть новый frontend-код` | границы FSD |
| `## 10. Что запрещено` | блокирующие анти-паттерны |
| `## 14. Self-check` | обязательные проверки |
| `## 16. Протокол завершения работы` | handoff |

**По требованию — только если unit это затрагивает:**

| Что делает unit | Дополнительно читать |
|---|---|
| новый слой/файл, границы `api`/`app`/`features`/`hooks`/`types`/`lib`/`ui` | `## 3. Границы слоев` |
| permissioned UI, scopes, access | `## 4. API Access и Scopes` |
| таблицы, сортировка, пагинация | `## 5. Таблицы, сортировка и пагинация` |
| фильтры | `## 6. Фильтры` |
| документационные вкладки | `## 7. Документационные вкладки` |
| стили, UI Kit | `## 8. UI Kit и стили`, `## 14.1. Code style и ESLint` |
| новый API-файл, сервис, хук, компонент, страница | `## 9. Паттерны` |
| именование | `## 11. Именование` |
| выбор библиотек | `## 12. Технологический стек` |
| любые тесты | `## 13. Обязательное тестирование CMS frontend`, `## 15. Тестирование` |

Если Router не передал явных `contextFiles`, читай их из `openspec instructions apply --change <change> --json` и ограничься теми, что относятся к твоему unit.
...
5. Продолжай до завершения **назначенного execution unit** без ожидания дополнительных инструкций, если нет конкретного блокера. Завершив unit, верни Router handoff и остановись — не переходи к следующему unit'у самостоятельно.

Единица твоей работы — **один execution unit**, а не deliverable, не секция `### Frontend` и не весь change.

**Бюджет и circuit breaker.** Бюджет одного unit (норматив в `AGENTS.md`): один архитектурный slice (например `E1` реализация UI, `E2` автоматизированные тесты, `E3` browser/manual QA — три разных unit'а), примерно **8–12 существенных действий**, **одна группа verification**. Если unit не помещается в бюджет: выполни безопасную атомарную часть, отметь только реально выполненное и верни handoff со статусом `partial` и предложением split вида `FE-2 → FE-2a + FE-2b`. Героически доводить перегруженный unit до конца в одной сессии запрещено.
...
Это минимумы **строк test matrix** из `design.md`, а не отдельных checklist-пунктов: в `tasks.md` они сгруппированы в одну задачу на компонент/фичу.
...
0. Убедись, что закрываешь **именно назначенный execution unit**, а не расширенный scope.
...
Формат handoff (см. `AGENTS.md`) — короткий, без пересказа плана и без построчного перечисления тест-сценариев: ссылайся на ID из `design.md` → `## Test matrix`.

```text
Unit: <ID> — <название> | Профиль: Frontend | Статус: done / partial / blocked
Задача: <краткое описание>
Изменены файлы: <список>
Verification: npm test → <результат>; npm run lint → <результат>; npx tsc --noEmit → <результат>; npm run build → <результат>
Тесты: <ID из test matrix>
...
Остаток / split: <нет | FE-2 → FE-2a + FE-2b с границами>
```

Этот handoff является evidence исполнителя, но не Quality Gate verdict. Формальный review выполняется по lane-модели (`QG-BE`, `QG-FE`, `QG-CONTRACTS`, `QG-LIVE`, `QG-SYNTH`) после завершения всех профильных units; blocking findings возвращаются владельцу как новые execution units, а исправления проверяются повторным прогоном затронутых lanes.
