...
- не добавляешь новый код без тестов или явного test-gap отчета, если задача меняет поведение;
...
| Icons | Для нового кода использовать `@ant-design/icons` по умолчанию |
| MUI | Не расширять использование MUI icons/components без причины и плана |
...
| UI runtime | React 19 |
| UI kit | Ant Design |
| Icons | `@ant-design/icons` для нового кода |
...
| Markdown diff | `git diff --check -- agents/frontend.md docs/plans/frontend_agent_improving_26_05_11.md` | Whitespace/errors в документационных правках |

Обязательные проектные команды для frontend-code diff:
...
Новый код без тестов считается ошибкой, если он меняет поведение. Сейчас `services/frontend/package.json` может не содержать test runner; до добавления инфраструктуры обязательны static checks, lint, typecheck и build для code diff.

| Уровень | Инструмент | Что проверять |
|---|---|---|
| Static/self-check | `rg`, ESLint, TypeScript, Next build | Архитектурные запреты, типы, сборка |
| Unit | Vitest + React Testing Library после отдельного добавления зависимостей | Hooks/services, filter normalization, permission helpers, pagination mapping, table sort mapping |
| Component | React Testing Library | Таблицы, фильтры, reusable pagination control, модалки, disabled/hidden actions by scopes |
| API boundary mocks | MSW после отдельного добавления зависимостей | Success/error/loading, `401/403`, refresh expectations, DTO/query serialization |
...
3. Запусти применимые self-check/test команды или явно объясни, почему они не применимы.
4. Укажи test gaps и инфраструктурные ограничения, если они есть.
5. Не оставляй незавершенные migration steps без плана.
