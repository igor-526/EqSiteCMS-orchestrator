...
- **WHEN** authenticated пользователь пытается изменить code или другие данные лошади вне разрешённого tenant context
