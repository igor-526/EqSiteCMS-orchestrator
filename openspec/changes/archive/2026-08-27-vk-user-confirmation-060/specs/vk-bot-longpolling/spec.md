...
### Requirement: Fail-fast при отсутствии токена

Bot runtime MUST завершаться с кодом ошибки и понятным сообщением, если `VK_GROUP_TOKEN` пуст или содержит placeholder-значение. Отсутствие токена MUST NOT влиять на работоспособность HTTP-контура `vk-service`.
