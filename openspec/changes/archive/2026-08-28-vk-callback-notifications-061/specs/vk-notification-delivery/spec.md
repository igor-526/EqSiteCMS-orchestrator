...
Notification Service MUST публиковать `NotificationVkCommand` в subject `commands.notification.vk.send` существующего stream `NOTIFICATION_COMMANDS`. Payload MUST содержать `occurred_at`, `event_uuid`, `callback_request_id`, непустой уникальный массив `user_ids` и непустой plain-text `text`, MUST запрещать дополнительные поля и MUST использовать `Nats-Msg-Id=callback_request_id`. Notification Service producer schema и VK Service consumer schema MUST быть идентичны.
