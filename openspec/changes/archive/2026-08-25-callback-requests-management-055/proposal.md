...
- Добавить в `services/backend` постоянное хранение заявок и реестр двух числовых статусов с seed-данными: `1` («Новая») и `2` («Обработана»); установка spam также переводит заявку в состояние `2`.
...
- Добавить защищённые CMS API для списка/детали, справочника статусов и разрешённых изменений только `status`/`is_spam`, а также служебные команды для `status`, `is_spam` и `notifications_delivered`; delivery-флаг выставляется после успешной публикации notification-service downstream email command и не ожидает SMTP acknowledgement. Удаления и редактирования контактных данных не предоставлять.
