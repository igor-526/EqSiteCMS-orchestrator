...
**Reason**: Утверждённая trust model не использует peer authentication между private микросервисами; старое требование создавало credentialed deputy.
**Migration**: Сначала закрыть host exposure email/notification и подтвердить private compose network, затем удалить `BACKEND_SERVICE_KEY` из backend→email client/config и downstream validation. `X-Service-Key` сохраняется только для microservice→main-backend `/api/service/...`.
