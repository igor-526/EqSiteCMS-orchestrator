...
- [ ] 1.4 Обновить `models/__init__.py` с экспортом моделей

## 2. Database Migration & Seeding

- [ ] 2.1 Создать миграцию `20260814_0001_create_notification_tables.py`
...
- [ ] 2.2 Создать модуль сидирования (по аналогии с backend)
...
- [ ] 2.6 Обновить `_build_seeders` в `utils/seeding/init_registry.py`
  - Добавить ChannelSeeder и EventSeeder

## 3. Repositories

- [ ] 3.1 Создать `repositories/base.py` с базовым `BaseRepository`
  - Методы: get_by_id, get_all, create, update, delete

- [ ] 3.2 Создать `repositories/channel.py` с `ChannelRepository`
  - Методы: get_by_code, get_active_channels

- [ ] 3.3 Создать `repositories/event.py` с `EventRepository`
  - Методы: get_by_code, get_active_events

- [ ] 3.4 Создать `repositories/user_notification_setting.py` с `UserNotificationSettingRepository`
  - Методы: get_by_user_and_event, get_users_by_event

- [ ] 3.5 Обновить `repositories/__init__.py` с экспортом репозиториев

## 4. Service Layer

- [ ] 4.1 Создать `core/services/notification_orchestrator.py` с `NotificationOrchestratorService`
...
- [ ] 4.2 Создать `core/services/event_handler_registry.py` с `EventHandlerRegistry`
  - Registry для маппинга event_code → handler_class
  - Метод: `get_handler(event_code)`

- [ ] 4.3 Создать `core/services/handlers/callback_handler.py` с `CallbackEventHandler`
...
- [ ] 4.4 Обновить `core/services/callback_request.py`
  - Интеграция с `NotificationOrchestratorService`
  - Удаление хардкода email
  - Получение email из настроек пользователя

- [ ] 4.5 Обновить `core/services/__init__.py` с экспортом сервисов

## 5. DI Container

- [ ] 5.1 Обновить `containers/database.py` с подключением к PostgreSQL
  - AsyncSession factory
  - Репозитории как Singleton

- [ ] 5.2 Обновить `containers/application.py`
...
- [ ] 6.1 Обновить `clients/nats/handlers/callback_request.py`
  - Интеграция с `NotificationOrchestratorService`
  - Удаление прямого вызова `CallbackRequestService`

- [ ] 6.2 Обновить `clients/nats/consumers/callback_request.py`
  - Обновление DI зависимостей

## 7. Settings & Configuration

- [ ] 7.1 Обновить `settings.py`
...
- [ ] 7.2 Создать `.env.example` с примерами переменных окружения

## 8. Testing

- [ ] 8.1 Создать `tests/unit/repositories/test_channel_repository.py`
  - Тесты CRUD операций
  - Тесты get_by_code, get_active_channels

- [ ] 8.2 Создать `tests/unit/services/test_notification_orchestrator.py`
  - Тесты пайплайна обработки событий
  - Mock зависимостей

- [ ] 8.3 Создать `tests/integration/test_callback_flow.py`
  - Интеграционный тест полного пайплайна
  - Mock main backend

## 9. Documentation

- [ ] 9.1 Обновить `README.md` с описанием notification-service
  - Архитектура, настройка, запуск

- [ ] 9.2 Создать `docs/notification-service.md` с детальным описанием
