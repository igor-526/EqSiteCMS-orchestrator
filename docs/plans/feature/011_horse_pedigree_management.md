...
В `docs/plans/feature/010_horses_management.md` уже реализован раздел CMS «Лошади» и `HorsePedigreeModal` как заглушка. Backend уже содержит:
...
Текущее поведение близко к нужному, но требует выравнивания с задачей `docs/tasks/011_horse_pedigree_management.md`:
