# Контекст
При работе с фотографиями были обнаружены следующие баги:
## Загрузка фотографий
Баг можно воспроизвести с помощью попытки загрузить фотографию со слишком длинным наименованием
```
...
```
## Удаление не добавленных фотографий
При добавлении фотографий в галерею есть модальное окно, в котором можно быстро удалить добавленные фотографии
Frontend при удалении добавленной фотографии (которая закончилась вышеописанной ошибкой) выдал ошибку и попытался сделать такой запрос
```
INFO: 10.244.0.37:52916 - "DELETE /api/photos/temp-1787064931222-0.5688643649315095 HTTP/1.1" 422 Unprocessable Content
```
## Размер фотографии
При добавлении фотографии в галерею весом более 1МБ позникает такая ошибка, которую отдаёт nginx
```
413 Content Too Large
```
Посмотри templates чарта backend и доработай ingress, чтобы можно было добавлять фотографии весом 10МБ и более
